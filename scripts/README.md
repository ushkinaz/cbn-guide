...
- **`transifex-download.ts`** (`pnpm i18n:download`): Downloads resource translations per locale using `resource_translations_async_downloads` API and saves locale JSON files.
- **`transifex-upload.ts`** (`pnpm i18n:upload`): Updates existing resource translations using bulk `PATCH /resource_translations` from local JSON files (for AI-assisted translation workflow).
