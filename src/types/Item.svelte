...
const staticItem: Item = untrack(() => item);

const weaponCategories = asArray(staticItem.weapon_category);

const _context = "Item Basic Info";

const qualities = (staticItem.qualities ?? []).map(([id, level]) => ({
...
const materials =
  staticItem.material == null
    ? []
    : typeof staticItem.material === "string"
      ? [{ type: staticItem.material, portion: 1 }]
      : isStrings(staticItem.material)
        ? staticItem.material.map((s) => ({ type: s, portion: 1 }))
        : staticItem.material.map((s) => ({
            type: s.type,
            portion: s.portion ?? 1,
          }));
...
const flags = [staticItem.flags ?? []]
  .flat()
  .map((id) => data.byIdMaybe("json_flag", id) ?? { id });
const faults = (staticItem.faults ?? []).map((f) => data.byId("fault", f));

const uncraftRecipe = data.uncraftRecipe(staticItem.id);
...
const vparts = data
  .byType("vehicle_part")
  .filter((vp) => vp.id && vp.item === staticItem.id);

const usage: UseFunction[] = normalizeUseAction(staticItem.use_action).map(
  (s) => {
...
const ascii_picture =
  staticItem.ascii_picture &&
  data.byIdMaybe("ascii_art", staticItem.ascii_picture);
...
const fuelForVPs = data
  ...
  .filter(
    (vp) =>
      vp.id &&
      (vp.fuel_options?.includes(staticItem.id) ||
        vp.fuel_type === staticItem.id),
  );
...
const usedToRepair = data.byType("fault").filter((f) => {
  ...
  return mendingMethods.some((mm) =>
    mm.components.some((c) => c.some((i) => i.id === staticItem.id)),
  );
});

const grantedByMutation = data
  .byType("mutation")
  .filter((m) => m.id && m.integrated_armor?.some((x) => x === staticItem.id));
