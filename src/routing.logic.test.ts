...
describe("Routing logic", () => {
  ...
  describe("URL parsing and encoding", () => {
    ...
    test("normalizes mods query param in parseRoute", () => {
      ...
      expect(parseRoute().mods).toEqual(["aftershock", "magiclysm"]);
    });
    ...
    test("mods query param round-trips through buildUrl and parseRoute", () => {
      ...
      expect(parseRoute()).toMatchObject({
    ...
    test.each([
      ...
      "special@#$chars",
      "unicode: 日本語",
    ])("round-trips search query %s", (query) => {
      ...
      expect(parseRoute().search).toBe(query);
    });
    ...
    test.each(["simple_id", "id with spaces", "item_id+variant", "a+b"])(
      ...
      (id) => {
        ...
        expect(parseRoute().item?.id).toBe(id);
      },
    );
  });
